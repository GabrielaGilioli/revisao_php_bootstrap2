...
if (empty($dados['nome'])) {
    ...
    //Se o campo numero estiver vazio, retorna mensagem de erro
    ...
    if ($edit_usario->execute()) {
         //Editar no bancode dados na segunda tabela (enderecos)
         $query_endereco = $conn->prepare($query_endereco);
         ...
         if($edit_endereco->execute()) {
            $retorna = ['status' => true, 'msg' => "<div class='alert alert-success' role='alert'>Usuário editdo com sucesso!</div>"];
         } else {
